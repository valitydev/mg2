...
handle_info({'DOWN', MRef, process, Self, Reason}, #st{self = Self, monitor = MRef} = St) ->
...
defer_heartbeat(#st{timer = undefined, opts = #{heartbeat := HeartbeatOpts}} = St) ->
    Msg = heartbeat,
...
handle_heartbeat(TRef, #st{timer = TRef, self = Self, members = Members, opts = Opts} = St) ->
