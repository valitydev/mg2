...
%% This type should actually belong to mg_core app
...
%% Trying to future-proof when we would want to merge this and mg_woody_event_sink:serialize
...
serialize_timesamp(Timestamp) ->
    mg_woody_packer:pack(timestamp_ns, Timestamp).
...
target_event(EventData, Timestamp) ->
    #mg_lifesink_LifecycleEvent{
        machine_ns = ?NS,
        machine_id = ?ID,
        created_at = mg_woody_packer:pack(timestamp_ns, Timestamp),
        data = EventData
    }.
