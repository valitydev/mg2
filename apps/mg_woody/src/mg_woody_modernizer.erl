...
-spec modernize_event(options(), woody_context:ctx(), mg_core_events_modernizer:machine_event()) ->
...
modernize_event(Options, WoodyContext, MachineEvent) ->
    ...
    Args = {mg_woody_packer:pack(machine_event, MachineEvent)},
    {ok, Result} = woody_client:call({Service, 'ModernizeEvent', Args}, Options, WoodyContext),
    mg_woody_packer:unpack(modernize_result, Result).
