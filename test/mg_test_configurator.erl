...
-spec api_event_sink_options(_, _) -> mg_woody_api_event_sink:options().
...
-spec tags_options(mg_core:ns(), events_machines()) -> mg_core_machine_tags:options().
...
modernizer_options(#{current_format_version := CurrentFormatVersion, handler := WoodyClient}) ->
    #{
        modernizer => #{
            ...
            handler =>
                {mg_woody_api_modernizer, WoodyClient#{
